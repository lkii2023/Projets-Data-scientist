...
puts "*" * width
puts "\n" 
puts "My program starts here:".center(width)
...
number_of_handles = handles.count

puts "the number of handles is: #{number_of_handles}"

puts "\n" 

puts "Quel est le handle le plus court de cette liste ?"

shortest_handle = handles.min_by(&:length)
puts "The shortest handle is: #{shortest_handle}"

puts "\n" 

puts "Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)"

count = handles.count { |handle| handle.length - 1 == 5 }
puts "Number of handles with 5 characters: #{count}"
...
count = handles.count { |handle| handle[1].match(/[A-Z]/) }
puts "Number of handles that start with an uppercase letter: #{count}"

sorted_handles = handles.first(20).sort_by {|handle| handle.gsub('@','')}
...
sorted_handles = handles.first(20).sort_by { |handle| handle.length }
...
search_handle = "@epenser"

position = handles.find_index(search_handle)

if position.nil?
  puts "#{search_handle} not found in the array"
else
  puts "#{search_handle} is at position #{position} in the array"
end
...
handle_lengths = handles.group_by {|handle| handle.length}
handle_lengths.each do |length, handle|
  puts "Number of handles with #{length} characters: #{handle.size}"
end
...
puts "My program ends here.".center(width)
puts "*" * width
